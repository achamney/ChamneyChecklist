...
function PythonAnywhereService() {
    ...
    this.setList = async function (items, listId, callback) {
        var cacheBuster = Math.floor(Math.random()*10000);
        var loadingIcon = $(".loading").show();
        return $.ajax({
            url: MASTERURL+"set/"+listId+"?cb="+cacheBuster,
            type: "POST",
    ...
    this.getList = async function(listId) {
        var loadingIcon = $(".loading").show();
        var cacheBuster = Math.floor(Math.random()*10000);
        var ret = await $.get(MASTERURL+listId+"?cb="+cacheBuster);
        loadingIcon.hide();
        return JSON.parse(ret);
    }
    this.getAllLists = async function() {
        var loadingIcon = $(".loading").show();
        var cacheBuster = Math.floor(Math.random()*10000);
        var ret = await $.get(MASTERURL+"2e8e0898-12f8-11f0-b5ae-0afff235d613"+"?cb="+cacheBuster);
        loadingIcon.hide();
        return ret;
    }
    this.makeNewList = async function(gamestate) {
        var loadingIcon = $(".loading").show();
        return await $.ajax({
            url: MASTERURL+"make",
            type: "POST",
